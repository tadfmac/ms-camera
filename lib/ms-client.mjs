...
export default class MSClient {
  constructor(socketClient, config){
    ...
    this.type = null;
    this.consumer = null;
//    this.onUpdateViewers = null;
//    this.onUpdateCasters = null;
//    this.onTransportDisconnected = null;
  }
    ...
    this.socketClient.on('connect', async (socket) => {
      ...
      try {
        ...
        if(this.type == "caster"){
          ...
          for(let cnt=0;cnt<_cams.length;cnt++){
            this.caster.cams[_cams[cnt]] = {id:_cams[cnt], producer:null, viewers:{}};
          }
  ...
  async produceCam(stream,camid) {
    ...
    try {
      if(camid in this.caster.cams){
        ...
        console.log("invalid cam id : "+cam);
      }
    } catch (err) {
  ...
  async startConsume(casterId,camName) {
    try {
      ...
      if(data){ // produceまだされてないこともあるので
        ...
        let codecOptions = {};
        this.consumer = await this.receiveTransport.consume({id,producerId,kind,rtpParameters,codecOptions});
