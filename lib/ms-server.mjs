...
export default class MediasoupServer {
    ...
    this.socketServer.on('connection', (socket) => {
      ...
      socket.on('disconnect', async () => {
        ...
        if(this.cons[socket.id].caster != null){
          ...
          Object.keys(this.casters[caster.id].cams).forEach((camName)=>{
            Object.keys(this.casters[caster.id].cams[camName].viewers).forEach(async (viewer)=>{
      ...
      socket.on('produce', async (data, callback) => {
        ...
        if(camid in this.casters[caster].cams){
          ...
          this.casters[caster].cams[camid] = {id:camid, producer:null, viewers:{}};
        }
          ...
          if(camid in this.casters[caster].cams){
            if(this.casters[caster].cams[camid].producer != null){
              Object.keys(this.casters[caster].cams[camid].viewers).forEach((viewer)=>{
                if((this.viewers[viewer].casterId == caster)&&(this.viewers[viewer].camName == camid)){
                  if(this.viewers[viewer].consumer != null){
                    this.socketServer.to(this.viewers[viewer].socketid).emit("consumerClosed",{casterId:caster, camName:camid});
  ...
  _makeCasterArr(){
    ...
    Object.keys(this.casters).forEach((_id)=>{
      ...
      Object.keys(this.casters[_id].cams).forEach((_cam)=>{
        if(this.casters[_id].cams[_cam].producer != null){
          _cams.push(_cam);
        }
      });
      if(_cams.length > 0){
  ...
  async createConsumer(casterId, camName, viewer, rtpCapabilities) {
    ...
    try {
      ...
      this.sendUpdateViewer(casterId,camName);
  ...
  sendUpdateViewer(_casterId,_camName){
    console.log("sendUpdateViewer() : casterId: "+_casterId+" camName: "+_camName);
    let data = {camName:_camName, viewers:{}};
    let id = this.casters[_casterId].socketid;
    Object.keys(this.casters[_casterId].cams[_camName].viewers).forEach((viewer)=>{
